$(document).ready(function () {
  ...
  $(".nav-item.drop-list-data .nav-link").on('click', function(event) {
    ...
    var currentItem = $(this).closest('.nav-item');
    
    // إغلاق جميع القوائم الأخرى
    $('.nav-item.drop-list-data').not(currentItem).removeClass("active");
    
    // فتح/إغلاق القائمة الحالية
    currentItem.toggleClass("active");
  });

  // إغلاق القوائم المنسدلة عند الضغط في أي مكان آخر
  $(document).on('click', function(event) {
    if (!$(event.target).closest('.nav-item.drop-list-data').length) {
      $('.nav-item.drop-list-data').removeClass("active");
    }
  });
